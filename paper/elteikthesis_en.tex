...
% Document's metadata
\title{Áron Menő Szakdogája} % title
\date{2023} % year of defense

% Author's metadata
\author{Tatai Áron Péter}
\degree{Programtervező Informatikus BSc}

% Superivsor(s)' metadata
\supervisor{Illés Zoltán habil.} % internal supervisor's name
\affiliation{Assistant Lecturer} % internal supervisor's affiliation
\extsupervisor{Balogh György} % external supervisor's name
\extaffiliation{CTO, Developer} % external supervisor's affiliation

% University's metadata
\university{Eötvös Loránd University} % university's name
\faculty{Faculty of Informatics} % faculty's name
\department{Dept. of ZSAA} % department's name
\city{Budapest} % city
